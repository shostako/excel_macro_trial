Attribute VB_Name = "m転記_段取塗装"
Option Explicit

' ========================================
' マクロ名: 転記_段取塗装
' 処理概要: 段取塗装テーブルから集計表T列へ段取データを転記
' ソーステーブル: テーブル「_段取塗装」（シート不問、テーブル名で検索）
' 基準日付: シート「集計表」セルA1の日付
' 転記先: シート「集計表」T列（行20-25）
...
Sub 転記_段取塗装()
    ...
    Application.StatusBar = "段取塗装データの転記を開始します..."
    ...
    ' ソーステーブル（_段取塗装）をブック全体から検索
    Dim sourceTable As ListObject
    Dim ws As Worksheet
    Dim lo As ListObject
    For Each ws In ThisWorkbook.Worksheets
        For Each lo In ws.ListObjects
            If lo.Name = "_段取塗装" Then
                Set sourceTable = lo
                Exit For
            End If
        Next lo
    ...
    If sourceTable Is Nothing Then
        MsgBox "テーブル「_段取塗装」が見つかりません。", vbCritical, "テーブルエラー"
        GoTo CleanupAndExit
    End If

    ' データ範囲の確認
    If sourceTable.DataBodyRange Is Nothing Then
        MsgBox "「_段取塗装」テーブルにデータがありません。", vbCritical, "データエラー"
        GoTo CleanupAndExit
    End If
    ...
    If Err.Number <> 0 Then
        MsgBox "「_段取塗装」テーブルに「日付」列が見つかりません。", vbCritical, "列エラー"
        GoTo CleanupAndExit
    End If
    On Error GoTo ErrorHandler
    ...
    Application.StatusBar = "段取塗装データ転記中..."
    ...
    transfers = Array( _
        ...
        Array("日段取時間", 21, 20), _
        Array("日段取回数", 22, 20), _
        Array("累計段取時間", 23, 20), _
        Array("累計段取回数", 24, 20), _
        Array("平均段取時間", 25, 20) _
    )
    ...
    ' 正常終了
    Application.StatusBar = "段取塗装データの転記完了"
