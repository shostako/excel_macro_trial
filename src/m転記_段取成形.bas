Attribute VB_Name = "m転記_段取成形"
Option Explicit

' ========================================
' マクロ名: 転記_段取成形
' 処理概要: 段取成形テーブルから集計表T列へ2号機・3号機の段取データを転記
' ソーステーブル: テーブル「_段取成形」（シート不問、テーブル名で検索）
' 基準日付: シート「集計表」セルA1の日付
' 転記先: シート「集計表」T列（2号機: 行4-9、3号機: 行11-16）
' 転記指標: 6種類×2号機（日平均時間、日段取時間、日段取回数、累計段取時間、累計段取回数、平均段取時間）
...
Sub 転記_段取成形()
    ...
    Application.StatusBar = "段取成形データの転記を開始します..."
    ...
    ' ソーステーブル（_段取成形）をブック全体から検索
    Dim sourceTable As ListObject
    Dim ws As Worksheet
    Dim lo As ListObject
    For Each ws In ThisWorkbook.Worksheets
        For Each lo In ws.ListObjects
            If lo.Name = "_段取成形" Then
                Set sourceTable = lo
                Exit For
            End If
        Next lo
    ...
    If sourceTable Is Nothing Then
        MsgBox "テーブル「_段取成形」が見つかりません。", vbCritical, "テーブルエラー"
        GoTo CleanupAndExit
    End If

    ' データ範囲の確認
    If sourceTable.DataBodyRange Is Nothing Then
        MsgBox "「_段取成形」テーブルにデータがありません。", vbCritical, "データエラー"
        GoTo CleanupAndExit
    End If
    ...
    If Err.Number <> 0 Then
        MsgBox "「_段取成形」テーブルに「日付」列が見つかりません。", vbCritical, "列エラー"
        GoTo CleanupAndExit
    End If
    On Error GoTo ErrorHandler
    ...
    Application.StatusBar = "段取成形データ転記中..."
    ...
    transfers = Array( _
        ...
        Array("日段取時間2", 5, 20), _
        Array("日段取回数2", 6, 20), _
        Array("累計段取時間2", 7, 20), _
        Array("累計段取回数2", 8, 20), _
        Array("平均段取時間2", 9, 20), _
        Array("日平均時間3", 11, 20), _
        Array("日段取時間3", 12, 20), _
        Array("日段取回数3", 13, 20), _
        Array("累計段取時間3", 14, 20), _
        Array("累計段取回数3", 15, 20), _
        Array("平均段取時間3", 16, 20) _
    )
    ...
    ' 正常終了
    Application.StatusBar = "段取成形データの転記完了"
