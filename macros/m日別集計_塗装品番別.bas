Attribute VB_Name = "m日別集計_塗装品番別"
Option Explicit

' 塗装品番別日別集計マクロ（空白値を0に変換）
' 「全工程」テーブルから「塗装」工程のデータを日付・品番でグループ化して集計
Sub 日別集計_塗装品番別()
    ' 高速化設定（最優先）
    ...
    outputSheetName = "日別集計_塗装品番別"
    outputTableName = "日別集計_塗装品番別テーブル"
    ...
    For i = 1 To UBound(dataArray, 1)
        ...
        ' 塗装工程のみ処理
        If dataArray(i, colProcess) = "塗装" Then
