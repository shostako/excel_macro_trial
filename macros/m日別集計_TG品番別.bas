Attribute VB_Name = "m日別集計_TG品番別"
Option Explicit

' TG品番別日別集計マクロ（空白値を0に変換）
' 「全工程」テーブルから「製造3」工程のデータを日付・品番でグループ化して集計
Sub 日別集計_TG品番別()
    ' 高速化設定（最優先）
    ...
    outputSheetName = "日別集計_TG品番別"
    outputTableName = "日別集計_TG品番別テーブル"
    ...
    For i = 1 To UBound(dataArray, 1)
        ...
        ' 製造3工程のみ処理
        If dataArray(i, colProcess) = "製造3" Then
            currentDate = dataArray(i, colDate)
            currentHinban = dataArray(i, colHinban)
            dictKey = Format(currentDate, "yyyy/mm/dd") & "_" & currentHinban
            ...
            If dict.Exists(dictKey) Then
                ...
                dict(dictKey) = item
            Else
                dict(dictKey) = Array(currentDate, currentHinban, jissekiVal, dandoriVal, kadouVal, furyoVal)
            End If
        End If
    Next i
