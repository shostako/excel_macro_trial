Attribute VB_Name = "m日別集計_成形号機別"
Option Explicit

' 成形号機別日別集計マクロ（空白値を0に変換）
' 「全工程_成形」テーブルから「成形」を含む工程のデータを日付・機械でグループ化して集計
Sub 日別集計_成形号機別()
    ' 高速化設定（最優先）
    ...
    Dim colDate As Long, colProcess As Long, colMachine As Long
    ...
    ' ========== 基本設定 ==========
    Set wb = ThisWorkbook
    sourceSheetName = "全工程_成形"
    sourceTableName = "全工程_成形テーブル"
    outputSheetName = "日別集計_成形号機別"
    outputTableName = "日別集計_成形号機別テーブル"
    ...
    colMachine = GetColumnIndex(tblSource, "機械")
    ...
    If colDate = 0 Or colProcess = 0 Or colMachine = 0 Or colJisseki = 0 Or colDandori = 0 Or colKadou = 0 Or colFuryo = 0 Then
    ...
    For i = 1 To UBound(dataArray, 1)
        ...
        ' 成形を含む工程のみ処理
        If InStr(dataArray(i, colProcess), "成形") > 0 Then
            currentDate = dataArray(i, colDate)
            currentMachine = dataArray(i, colMachine)
            dictKey = Format(currentDate, "yyyy/mm/dd") & "_" & currentMachine
            ...
            If dict.Exists(dictKey) Then
                ...
                dict(dictKey) = item
            Else
                dict(dictKey) = Array(currentDate, currentMachine, jissekiVal, dandoriVal, kadouVal, furyoVal)
            End If
        End If
    Next i
